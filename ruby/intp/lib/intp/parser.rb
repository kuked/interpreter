  ...
  class Parser
    ...
    def parse_program
      ...
      loop do
        ...
        program.statements << stmt unless stmt.nil?
    ...
    def parse_let_statement
      ...
      return nil unless expect_peek(Intp::Token::IDENT)

      stmt.name = Intp::Identifier.new(@cur_token, @cur_token.literal)

      return nil unless expect_peek(Intp::Token::ASSIGN)
    ...
    def register_prefix(type, fnc)
      @@prefix_parse_fns[type] = method(fnc)
    end

    def register_infix(type, fnc)
      @@infix_parse_fns[type] = method(fnc)
    end
